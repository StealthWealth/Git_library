...
<body>
	...
	<table>
		...
		<table width="100%" border="1">
			...
			<c:forEach items="${subject_bbin }" var="su" varStatus="status">
				<tr align="center">
					<td>${status.index+1 }</td>
					<td>${su.serial_number }</td>
					<td>${su.member.mobile_Phone }</td>
					<td>${su.member.member_name }</td>
					...
					<td><c:if test="${su.ispayment==0 }">
						...
						<c:if test="${su.ispayment==1 }">
							<font color="green"><b>已还款</b></font>
						</c:if></td>
					<td>
						<c:if test="${su.ispayment==0 }">
							<a href="/YJLC_E/updateSubject_bbin/${su.id }/${su.subject.id}"><b>还款</b></a>
						</c:if>
						<c:if test="${su.ispayment==1 }">
							<font color="green"><b>已还款</b></font>
						</c:if>
